function [route,numExpanded] = AStarGrid(input_map, start_coords, dest_coords, drawMap)
    ...
    while true
        ...
        % необходимо посетить каждого соседа рассматриваемой клетки и  
        % обновить значения массивов map, f, g и parent

        idxs(1) = sub2ind(size(f), i, j);
        if (i > 1)
            idxs(end + 1) = sub2ind(size(f), i - 1, j);
        end
        if (i < nrows)
            idxs(end + 1) = sub2ind(size(f), i + 1, j);
        end
        if (j > 1)
            idxs(end + 1) = sub2ind(size(f), i, j - 1);
        end
        if (j < ncols)
            idxs(end + 1) = sub2ind(size(f), i, j + 1);
        end
